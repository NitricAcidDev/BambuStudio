    ...
    , m_show_history_dialog(show)
{
    ...
    mew_btn->SetBorderColor(CaliHistoryThemeGreen());
    ...
    m_extruder_switch_btn->SetBackgroundColour(CaliHistoryThemeGreen());
...
void HistoryWindow::sync_history_data() {
    ...
    // grid item num in row
    int column_count = column_idx + 1;
    // reset colume_idx
    column_idx = 0;
    ...
    for (auto& result : m_calib_results_history) {
        ...
        edit_button->SetBorderColor(CaliHistoryThemeGreen());
    ...
    , m_old_name(result.name)
{
    ...
    save_btn->SetBorderColor(CaliHistoryThemeGreen());
    ...
    , m_history_results(history_results)
{
    ...
    ok_btn->SetBorderColor(CaliHistoryThemeGreen());
...
void NewCalibrationHistoryDialog::on_select_nozzle_pos(wxCommandEvent &event)
{
    ...
    if (nozzle.IsNormal()) {
        ...
        auto nozzle_diameter = nozzle.GetNozzleDiameterType();

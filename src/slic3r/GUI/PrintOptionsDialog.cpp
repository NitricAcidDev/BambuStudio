...
static wxColour PrintOptionsThemeGreen() {
    auto *app = dynamic_cast<Slic3r::GUI::GUI_App*>(&Slic3r::GUI::wxGetApp());
    if (app && app->IsMainLoopRunning()) {
        return app->get_theme_colors().button_green.colorForStates(StateColor::Normal | StateColor::Enabled);
    }
    return PrintOptionsThemeGreen();
}

static wxColour PrintOptionsThemeGreenHovered() {
    auto *app = dynamic_cast<Slic3r::GUI::GUI_App*>(&Slic3r::GUI::wxGetApp());
    if (app && app->IsMainLoopRunning()) {
        return app->get_theme_colors().button_green.colorForStates(StateColor::Hovered | StateColor::Enabled);
    }
    return PrintOptionsThemeGreenHovered();
}

static wxColour PrintOptionsThemeGreenPressed() {
    auto *app = dynamic_cast<Slic3r::GUI::GUI_App*>(&Slic3r::GUI::wxGetApp());
    if (app && app->IsMainLoopRunning()) {
        return app->get_theme_colors().button_green.colorForStates(StateColor::Pressed | StateColor::Enabled);
    }
    return PrintOptionsThemeGreenPressed();
}

static StateColor btn_bg_green(std::pair<wxColour, int>(wxColour(194, 194, 194), StateColor::Disabled),
                               std::pair<wxColour, int>(PrintOptionsThemeGreenPressed(), StateColor::Pressed),
                               std::pair<wxColour, int>(PrintOptionsThemeGreenHovered(), StateColor::Hovered),
...
: DPIDialog(parent, wxID_ANY, _L("Printer Parts"), wxDefaultPosition, wxDefaultSize, wxCAPTION | wxCLOSE_BOX)
{
    ...
    m_wiki_link->SetForegroundColour(wxColour("#00AE42")); // Theme color applied later
    ...
    multiple_wiki_link->SetForegroundColour(wxColour("#00AE42")); // Theme color applied later
